...
for fhour in range(1,31,2):
	...
	overlay = []
	probabilityoverlay = np.zeros_like(reflectivities_copy[0])
						...
						if cv2.contourArea(interior_contour)>=100:
							...
							try:	
								...
								if (length>=250 and length/width>=3.0 and coldpts/allpts>=0.5):
									print('hit',length,width)
									overlay[interiorpixelpoints[1],interiorpixelpoints[0]] = 1
							except:
								continue
	
		probabilityoverlay[np.where(overlay>0)] += 1
	
	probabilityoverlay = (probabilityoverlay/8.0 + 0.0001)*100.
	probs = m.contourf(x,y,probabilityoverlay,cmap='Reds',levels=[0,12.5,25.0,37.5,50.0,62.5,75.0,87.5,100],vmin=0.0,vmax=100.0,zorder=3)
	
	#probs = m.contour(x,y,probabilityoverlay,cmap='Reds',levels=[0,12.5,25.0,37.5,50.0,62.5,75.0,87.5,100],vmin=0.0,vmax=100.0,linewidths=0.5,zorder=3)
	#plt.clabel(probs,levels=[0,12.5,25.0,37.5,50.0,62.5,75.0,87.5,100],fmt='%1.1f',inline=True,fontsize=6)
	...
	cbar = plt.colorbar(probs,fraction=0.025,ticks=[0,12.5,25.0,37.5,50.0,62.5,75.0,87.5,100])
	cbar.ax.yaxis.set_tick_params(color='w')
	cbar.ax.set_yticklabels([0,12.5,25.0,37.5,50.0,62.5,75.0,87.5,100],color='w')
	plt.box(False)

	outfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/href/%s%s00_snowbands_probability.png' % (datesub,str(fhour).zfill(2))

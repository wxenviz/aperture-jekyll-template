...
for fhour in range(0,31):
	...
	for i,reflectivity_copy in enumerate(reflectivities_copy):
		...
		reflectivity_plotting = np.copy(reflectivity_copy)
		reflectivity_plotting[reflectivity_plotting>1000000] = np.nan
		reflectivity_plotting[reflectivity_plotting>60] = 60.0
		reflectivity_plotting[reflectivity_plotting<=0] = np.nan
		im = m.imshow(reflectivity_plotting,zorder=2,aspect='equal',interpolation='none',cmap=cmap1,vmin=0,vmax=60.)
		...
		m.contour(x,y,overlay,colors='orange',linewidths=0.5,zorder=3)
	...
	outfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/href/%s%s00_snowbands_postage.png' % (datesub,str(fhour).zfill(2))

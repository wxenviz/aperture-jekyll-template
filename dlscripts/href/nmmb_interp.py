...
for fhour in range(0,31):
	print(fhour)
	backhr = fhour - 1
	fil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/%s%s00_nmmb.grib2' % (datesub,str(fhour).zfill(2))
	...
	if fhour==0:
		...
		for grb in grbs:
			if 'Total Precipitation' in str(grb) and 'fcst time %s' % (str(fhour-1)) in str(grb):
				hpcp = grb.values[:]
			elif 'Water equivalent' in str(grb) and 'fcst time %s' % (str(fhour-1)) in str(grb):
				hweasd = grb.values[:]
...
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/cref_nmmb.npy',crefs)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/bref_nmmb.npy',brefs)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/apcp_nmmb.npy',apcps)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/aweasd_nmmb.npy',aweasds)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/hpcp_nmmb.npy',hpcps)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/hweasd_nmmb.npy',hweasds)
np.save('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/href/temp_nmmb.npy',temps)

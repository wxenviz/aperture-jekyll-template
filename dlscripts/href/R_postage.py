...
for fhour in range(0,31):
	...
	for k,model in enumerate(models):
		...
		cref = np.load(cref_fil)[fhour]
		cref[cref<=0] = 0.0
		temp_mask = np.load(cref_mask)
		cref[temp_mask] = np.nan
		reflectivities.append(cref)
	...
	for i in range(0,8):
		...
		reflectivities[i][reflectivities[i]<=0.0] = np.nan
		reflectivities[i][reflectivities[k]>1000000] = np.nan
	...
	postagefil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/href/%s%s00_R_postage.png' % (datesub,str(fhour).zfill(2))

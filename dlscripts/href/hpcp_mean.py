...
for fhour in range(0,31):
	...
	hpcps_mean = np.mean(hpcps_copy,axis=0)
	hpcps_mean[hpcps_mean<0.01] = np.nan
	hpcps_mean[hpcps_mean>1000000] = np.nan
	hpcps_mean[hpcps_mean>1] = 1.0

	bounds = np.linspace(0,1,11)
	norm = colors.BoundaryNorm(boundaries=bounds, ncolors=256)
	im = m.imshow(hpcps_mean,zorder=2,norm=norm,cmap='jet',aspect='equal',interpolation='none',vmin=0.0,vmax=1.0)
	...
	plt.box(False)
	meanfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/href/%s%s00_hpcp_mean.png' % (datesub,str(fhour).zfill(2))

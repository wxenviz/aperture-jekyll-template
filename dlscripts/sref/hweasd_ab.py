...
totalsnowback = np.load('/gpfs_backup/stormtrack/jtradfor/ensemble_data/rawdata/sref/%s_asnow.npy' % (forecasthoursubback))
...
totalsnowback_copy = []
for snow in totalsnowback:
	snow[totalsnow_mask] = np.nan
	snow[snow>1000000] = np.nan
	totalsnowback_copy.append(snow)
...
totalsnow_ab[totalsnow_ab>2.0] = 2.0

im = m.imshow(totalsnow_ab,zorder=2,aspect='equal',interpolation='none',cmap='Blues',vmin=0.0,vmax=2.0)
...
plt.box(False)
sdfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/sref/%s_hweasd_ab.png' % (forecasthoursub)

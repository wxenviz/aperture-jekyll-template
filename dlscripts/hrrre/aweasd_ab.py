...
def asnow_ab(totalsnow,outname):
    ...
    totalsnow_ab[totalsnow_ab>1.2] = 1.2
    totalsnow_ab[-50:,:] = np.nan
    totalsnow_ab[:50,:] = np.nan
    totalsnow_ab[:,:50] = np.nan
    totalsnow_ab[:,-50:] = np.nan
    im = m.imshow(totalsnow_ab,aspect='equal',interpolation='none',cmap='Blues',vmin=0.0,vmax=1.2)
    plt.colorbar(im,fraction=0.023,pad=-0.02)
    plt.box(False)
    sdfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/hrrre/' + outname + '_aweasd_ab.png'
...
if len(datapaths)>0:
	...
	asnow_ab_fil = asnow_ab(totalsnow,fil)

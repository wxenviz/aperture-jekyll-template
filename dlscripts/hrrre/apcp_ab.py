...
def apcp_ab(totalPrecips,outname):
    ...
    for i in range(0,len(totalPrecips_ab)):
        for j in range(0,len(totalPrecips_ab[0])):
            ...
            for k in range(0,3):
                for l in range(0,3):
                    if i%3==k and j%3==l:
                        totalPrecips_ab[i,j] = totalPrecips_copy[c][i,j]
                    c+=1
                
    totalPrecips_ab[totalPrecips_ab<0.25] = np.nan
    totalPrecips_ab[totalPrecips_ab>100.0] = 100.0
    totalPrecips_ab[-50:,:] = np.nan
    totalPrecips_ab[:50,:] = np.nan
    totalPrecips_ab[:,:50] = np.nan
    totalPrecips_ab[:,-50:] = np.nan
    m.imshow(totalPrecips_ab,zorder=2,aspect='equal',interpolation='none',cmap='jet',vmin=0.0,vmax=100.0)
    plt.colorbar(fraction=0.023,pad=-0.02)
    plt.box(False)
    sdfil = '/gpfs_backup/stormtrack/jtradfor/ensemble_data/wxenviz.github.io/uploads/outimages/hrrre/' + outname + '_apcp_ab.png'
...
if len(datapaths)>0:
	...
	apcp_ab_fil = apcp_ab(totalPrecips,fil)
